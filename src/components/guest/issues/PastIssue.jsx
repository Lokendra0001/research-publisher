...
const PastIssue = () => {
  ...
  const [selectedVolIssue, setSelectedVolIssue] = useState({
    volId: "10fc7ffa-726c-49e4-9128-b82af176fe5f",
    issueId: "bb3b383b-6cbd-427f-8544-2435728766f9",
  });
    ...
    try {
      ...
      if (selectedVolIssue?.volId && selectedVolIssue?.issueId) {
        const res = await axios.get(
          `${constant.SERVER_URL}public/get-pastIssuePapers?vId=${selectedVolIssue.volId}&iId=${selectedVolIssue.issueId}`,
          { withCredentials: true },
        );
        setPapers(res.data.data);
        setCurrentPage(1);
        setLastFetchedIds({
          volId: selectedVolIssue.volId,
          issueId: selectedVolIssue.issueId,
        });
      }
    } catch (error) {
            ...
            <div className="space-y-6">
              {loading ? (
                ...
                [...Array(3)].map((_, index) => (
                  <div
                    key={index}
                    className="bg-white p-6 rounded-lg shadow-sm border border-border animate-pulse"
                  >
