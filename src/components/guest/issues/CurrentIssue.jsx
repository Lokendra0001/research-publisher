...
const CurrentIssue = () => {
    ...
    useEffect(() => {
        const fetchCurrentIssue = async () => {
            try {
                const res = await axios.get(`${constant.SERVER_URL}public/current-issue`);
                if (res.data.status) {
                ...
                <div className="w-full">
                    <div className="space-y-6 animate-pulse w-full">
                        ...
                        {Array.from({ length: 3 }).map((_, index) => (
                            <div key={index} className="border border-border bg-gray-50 rounded-md overflow-hidden shadow-sm">
            ...
            <div className="w-full">
                {data ? (
                    ...
                    <div className="text-center py-10">
                        <h2 className="text-xl font-semibold text-text-primary">Current Issue Not Available</h2>
                        <p className="text-text-primary/60 mt-2">The latest volume and issue have not been published yet.</p>
                    </div>
                )}
            </div>
        </div>
