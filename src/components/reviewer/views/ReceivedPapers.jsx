...
import { Clock, Download, Check, X, AlertTriangle, FileText } from 'lucide-react';
...
const ReceivedPapers = () => {
    ...
    const fetchAssignments = async () => {
        try {
            ...
            const res = await axios.get(`${constant.SERVER_URL}reviews/assignments?status=accepted&decision=pending`, { withCredentials: true });
            if (res.data.success) {
    ...
    const handleSubmitReview = async (id, decision) => {
        try {
            ...
            if ((decision === 'minor_reject' || decision === 'rejected') && !reason.trim()) {
            ...
            const res = await axios.post(`${constant.SERVER_URL}reviews/${id}/submit`, payload, { withCredentials: true });
                ...
                <table className="w-full min-w-[1200px] text-sm text-left text-text-primary/50">
                    <thead className="text-xs text-gray-700 uppercase bg-muted/50 border-b  border-border">
                        <tr>
                            ...
                            <th scope="col" className="px-4 py-3 font-bold w-[152px]">Paper Title</th>
                            <th scope="col" className="px-4 py-3 font-bold w-[152px] ">Date of Issue</th>
                            <th scope="col" className="px-4 py-3 font-bold w-[180px]">Deadline of Paper</th>
                            <th scope="col" className="px-4 py-3 font-bold w-20 text-center ">Paper</th>
                            <th scope="col" className="px-4 py-3 font-bold w-64 ">Reason</th>
                            <th scope="col" className="px-4 py-3 font-bold w-[140px] text-center ">Minor Reject</th>
                            <th scope="col" className="px-4 py-3 font-bold text-center min-w-[240px]">Action</th>
                        </tr>
                    </thead>
                        ...
                        assignments.length === 0 ? (
                            ...
                            <tbody>
                                {assignments.map((item, index) => (
                                    <tr key={item.id} className="bg-primary-foreground border-b border-border/60 hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-4 py-4 w-[152px] align-middle border-r border-border/50">
                                            <p className="text-text-primary/90 font-medium line-clamp-2" title={item.paper?.title}>
                                                {item.paper?.title}
                                            </p>
                                        </td>
                                        ...
                                        <td className="px-4 py-4 w-[140px] text-center align-middle border-r border-border/50">
                                            <button
                                                onClick={() => handleSubmitReview(item.id, 'minor_reject')}
                                            ...
                                            >
                                                {processingId === item.id && actionType === 'minor_reject' ? (
                                        ...
                                        <td className="px-4 py-4 min-w-[240px] text-center align-middle">
                                            <div className="flex gap-2 justify-center">
                                                ...
                                                >
                                                    {processingId === item.id && actionType === 'accepted' ? (
                                                        <div className="w-3 h-3 border-2 border-primary-foreground border-t-transparent rounded-full animate-spin"></div>
                                                    ) : null}
                                                    Approve
                                                </button>
                                                ...
                                                >
                                                    {processingId === item.id && actionType === 'rejected' ? (
                                                        <div className="w-3 h-3 border-2 border-primary-foreground border-t-transparent rounded-full animate-spin"></div>
                                                    ) : null}
                                                    Reject
                                                </button>
