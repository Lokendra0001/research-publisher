...
const RoleContent = () => {
  ...
  const renderContent = () => {
    switch (page) {
      ...
      case "check_status":
        return <CheckStatus />;
      case "submitted_papers":
        return <SubmittedPapers />;
      case "under_review":
        return <UnderReview />;
      case "accepted_papers":
        return <AcceptedPapers />;
      case "minor_rejected":
        return <MinorRejected />;
      case "rejected_papers":
  ...
  return (
    <div className={`flex flex-col lg:flex-row bg-white min-h-[calc(100vh-64px)] ${page === "payment_service" ? "bg-muted/5" : ""}`}>
      {page !== "payment_service" && <AuthorSidebar />}
      <main className={`${page === "payment_service" ? "w-full" : "flex-1 mt-5 md:p-8"}`}>
        {renderContent()}
      </main>
    </div>
  );
