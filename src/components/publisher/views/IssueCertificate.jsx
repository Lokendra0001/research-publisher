...
const IssueCertificate = () => {
    ...
    useEffect(() => {
        const fetchPapers = async () => {
            try {
                setLoading(true);
                const res = await axios.get(`${constant.SERVER_URL}admin/papers?status=published&certificateGenerated=false`, { withCredentials: true });
    ...
    const handleIssueCertificate = async () => {
        ...
        try {
            ...
            const data = papers.filter((paper) => selectedPapers.hasOwnProperty(paper.id));
            const res = await axios.post(`${constant.SERVER_URL}admin/generate-certificate`, data, { withCredentials: true })
            if (res.data.success) {
                notify.success(res.data.message);
                setPapers(papers.filter(paper => !selectedPapers.hasOwnProperty(paper.id)));
                setSelectedPapers({})
            }
        } catch (error) {
            notify.error(error?.response?.data?.message || error.message || "Failed to Issue Certificate!")
        } finally {
        ...
        <div className="bg-primary-foreground p-2 md:p-6 shadow-sm rounded-lg border border-border min-h-[500px]">
                ...
                <table className="w-full text-sm text-left text-text-primary/50">
                    <thead className="text-xs text-text-primary/70 uppercase bg-muted/50 border-b border-border">
                        <tr>
                            ...
                            <th scope="col" className="px-4 py-3 font-bold min-w-32">Paper ID</th>
                            <th scope="col" className="px-4 py-3 font-bold min-w-20">Title</th>
                            <th scope="col" className="px-4 py-3 font-bold min-w-40">Author Name</th>
                            ...
                            <th scope="col" className="px-4 py-3 font-bold min-w-20 text-center">Select</th>
                        </tr>
                    </thead>
                            ...
                            {filteredPapers.map((item, index) => (
                                <tr key={item.id} className="bg-primary-foreground border-b border-border/50 hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-4 py-4 align-middle">
                                        <p className="text-text-primary font-medium line-clamp-1" title={item.title}>
                                            {item.title}
                                        </p>
                                    </td>
                                    <td className="px-4 py-4 align-middle">
            ...
            {Object.values(selectedPapers).filter(Boolean).length > 0 && (
                <div className='flex justify-center py-5 w-full'>
                    ...
                    >
                        {isIssueCertificate ? "Issuing Certificate..." : `Issue Certificate (${Object.values(selectedPapers).filter(Boolean).length})`}
                    </button>
                </div>
            )}
        </div>
