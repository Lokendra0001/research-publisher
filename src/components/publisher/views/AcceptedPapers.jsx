...
const AcceptedPapers = () => {
    ...
    const fetchAssignments = async () => {
      try {
        ...
        const res = await axios.get(
          `${constant.SERVER_URL}admin/assignments?status=accepted&decision=accepted`,
  ...
  const filteredAssignments = assignments.filter(
    (item) =>
      ...
      (item.paper?.customId &&
        item.paper?.customId.toLowerCase().includes(filter.toLowerCase())),
  );
  ...
  const handlePublish = async (reviewId, paperId) => {
    ...
    try {
      ...
      const res = await axios.patch(
        `${constant.SERVER_URL}papers/${paperId}`,
        { reviewId, status: "published", pubAt: true },
        { withCredentials: true },
      );
      ...
      if (res.data.success) {
        ...
        setAssignments(
          assignments.filter((assignment) => assignment.id !== reviewId),
        );
      }
    } catch (error) {
      notify.error(
  ...
  return (
    <div className="bg-primary-foreground p-2 md:p-6 shadow-sm rounded-lg border border-border min-h-[500px]">
      <div className="border-b border-border pb-2 mb-6 flex flex-col sm:flex-row justify-between md:items-center gap-4">
        <h1 className="text-2xl font-bold text-secondary">Accepted Papers</h1>
        ...
        <table className="w-full text-sm text-left text-text-primary/50">
          <thead className="text-xs text-text-primary/70 uppercase bg-muted/50 border-b border-border">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 font-bold min-w-[200px]">
                Paper Title
              </th>
              <th
                scope="col"
                className="px-6 py-3 font-bold w-50 whitespace-nowrap"
              >
                Reviewed By
              </th>
              <th scope="col" className="px-6 py-3 font-bold w-16 text-center">
                Paper
              </th>
              <th
                scope="col"
                className="px-6 py-3 font-bold w-16 text-center whitespace-nowrap"
              >
                Payment Ref
              </th>
              <th scope="col" className="px-6 py-3 font-bold w-16 text-center">
              ...
              <th scope="col" className="px-6 py-3 font-bold w-28 text-center">
                Publish
              </th>
            </tr>
          </thead>
                ...
                >
                  ...
                  <td className="px-6 py-4 w-16 text-center align-middle">
                    {item.paper?.paymentScreenshotUrl ? (
                      ...
                      <span
                        className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-danger/5 text-danger/40"
                        title="No payment uploaded"
                      >
                        <Upload className="w-4 h-4" />
                      </span>
                    )}
