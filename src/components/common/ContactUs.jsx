...
const ContactUs = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const res = await axios.post(
        `${constant.SERVER_URL}public/contact`,
        data,
  ...
  return (
    <div className="max-w-5xl mx-auto py-8">
      {/* Header */}
      <h1
        className={`text-2xl md:text-3xl font-bold mb-8 border-b-2  pb-2 inline-block ${textColor}`}
      >
        Contact Us
      </h1>
      ...
      <div className="bg-primary-foreground rounded-lg shadow-sm border-2 border-border p-4 md:p-8 mb-10 text-center relative overflow-hidden">
        <div
          className={`absolute top-0 left-0 w-full h-1 ${isHome ? "bg-primary" : "bg-secondary"}`}
        ></div>
        <h2 className="text-xl md:text-2xl font-bold text-danger mb-3 md:mb-6 uppercase tracking-wide">
          Contact Us
        </h2>

        <div className="text-sm md:text-base space-y-2 text-text-primary/70 font-medium">
          <p className="text-base md:text-lg text-text-primary font-bold">
            510 International Finance Centre, VIP Rd,
          </p>

          <p>Vesu, Surat - 394210, Gujarat</p>

          <div className="flex items-center justify-center gap-2 mt-4 text-black">
            <Phone className="w-4 h-4" />
            <span>Mobile: 7990463757</span>
          </div>

          <div className="flex items-center justify-center gap-2 text-black">
            <Mail className="w-4 h-4" />
            <span>E-mail: editor@ijarmy.com</span>
          </div>
        </div>
      </div>
